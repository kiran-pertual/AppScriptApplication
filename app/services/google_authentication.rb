...
class GoogleAuthentication
  ...
  def get_new_access_token
    begin
      ...
      puts "access token : #{tokens.token}"
      puts "refresh token : #{tokens.refresh_token}"
      values = {}
      @access_token = values["access_token"] = tokens.token
      @refresh_token = values["refresh_token"] = tokens.refresh_token
      File.open("tokens.yml","w") do |file|
        YAML.dump(values, file)
      end
    rescue Exception => e
